...
mod tests {
    ...
    /// activity, with the latest verb and entity names.
    #[rocket::async_test]
    async fn dashboard_returns_recently_active_students_ordering() {
        use crate::db::recently_active_students;
        ...
        // Emit an older row for bob, then a newer row for alice.
        let mut tx = db.pool.begin().await.unwrap();
        emit(
            &mut tx,
            NewActivity::new(Verb::SyllabusAssigned, coach).target_student(bob),
        )
        .await
        .unwrap();
        tx.commit().await.unwrap();
        ...
        let rows = recently_active_students(&db.pool, 10).await.unwrap();

        assert_eq!(rows.len(), 2, "both students appear");
        // alice's row was more recent, so she comes first.
        assert_eq!(rows[0].student_id, alice, "alice sorts first (more recent)");
        assert_eq!(
            rows[0].verb, "syllabus_graduated",
            "alice's latest verb is syllabus_graduated"
        );
        assert_eq!(rows[1].student_id, bob, "bob sorts second (older activity)");
        assert_eq!(
            rows[1].verb, "syllabus_assigned",
            "bob's latest verb is syllabus_assigned"
        );
    }
}
