...
/// about the technique/syllabus itself, not one student; each feed resolves
/// which students see it at read time (pull model). This replaces `emit_fanout`
/// for the broadcast verbs (video added/visibility, technique edited, syllabus
/// technique added/removed), so one coach action is one row.
