...
export interface NewTechniqueFormProps {
  ...
  /** HTML id for the <form> so an external footer button can submit via form={formId}. */
  formId: string;
  /**
   * When provided, render an "Add to global library too" switch bound to this state,
   * and create the technique with is_global = addToGlobal. When undefined, no switch is
   * rendered and the technique is created global (current library behaviour — pass no
   * is_global to createLibraryTechnique).
   */
  addToGlobal?: boolean;
...
export function NewTechniqueForm({
  ...
  const showGlobalSwitch = addToGlobal !== undefined;
  const globalSwitchId = `${formId}_add_to_global`;
  ...
  async function handleSubmit(values: FormValues) {
    try {
      const response = await createLibraryTechnique({
        ...
        ...(addToGlobal === undefined ? {} : { is_global: addToGlobal }),
      });
      ...
      {showGlobalSwitch && (
        <div className="flex items-center justify-between gap-4 rounded-md border border-border p-3">
          <div className="space-y-0.5">
            <Label htmlFor={globalSwitchId}>Add to global library too</Label>
            <p className="text-xs text-muted-foreground">
              Make this technique available across the whole library, not just
              here.
            </p>
          </div>
          <Switch
            id={globalSwitchId}
            checked={addToGlobal}
            onCheckedChange={(next) => onAddToGlobalChange?.(next)}
          />
        </div>
      )}
